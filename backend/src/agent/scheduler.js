...
async function evaluateGapRule(projectId, rule) {
  const { requires, mustLinkTo, linkType } = rule.ruleDefinition;
  const modelKey = NODE_MODEL_MAP[requires];
  if (!modelKey) return [];

  const nodes = await prisma[modelKey].findMany({ where: { projectId } });
  const links = await prisma.traceLink.findMany({
    where: { projectId, fromType: requires, toType: mustLinkTo, linkType },
  });
  const linkedFromIds = new Set(links.map((l) => l.fromId));
  ...
  return nodes
    ...
    .map((n) => ({
      ...
      nodeId: n.id,
      nodeTitle: n.title,
      message: `${requires} "${n.title}" has no linked ${mustLinkTo} (${linkType})`,
    }));
}
